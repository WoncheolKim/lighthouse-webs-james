...
const printTracks = function() {
  for (const key in library) {
    if (key === "tracks") {
      const ts = library.tracks;
      for (const t in ts) {
        console.log(`${t}: ${ts[t].name} by${ts[t].artist} (${ts[t].album})`);
      }
    }
  }
};
...
const addPlaylist = function(name) {
  ...
  library.playlists[pId] = {id: pId, name: name, track: []};
}
addPlaylist("moon");
